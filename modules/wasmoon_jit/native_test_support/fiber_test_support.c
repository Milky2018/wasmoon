...
static WASMOON_GUEST_ABI int hostcall_probe_trampoline(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
...
static WASMOON_GUEST_ABI int fiber_stack_hostcall_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
...
static WASMOON_GUEST_ABI int active_fiber_stack_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
...
static WASMOON_GUEST_ABI int nested_hostcall_tls_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
...
static WASMOON_GUEST_ABI int nested_activation_exception_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
    (void)func_ptr;
    sigjmp_buf *landing = exception_try_begin_impl(ctx, 17);
    int jumped = sigsetjmp(*landing, 1);
    if (jumped == 0) {
        int64_t slots[1] = {0};
        int result = wasmoon_jit_hostcall(
            ctx,
        ...
        exception_throw_impl(ctx, 1, NULL, 0);
        return 8;
    }
    exception_try_end_impl(ctx, 17);
    values[0] = jumped;
...
static WASMOON_GUEST_ABI int mismatched_try_end_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
...
static WASMOON_GUEST_ABI int nested_activation_gc_root_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
    ...
    int32_t gc_ref = encoded > 0 && (encoded & 1L) == 0
        ? (int32_t)(encoded >> 1)
        : 0;
    if (gc_collect_for_alloc_internal(ctx, NULL, 0) < 0) {
        ctx_gc_pop_root_scope_internal(ctx);
        return 9;
    }
...
static WASMOON_GUEST_ABI int parked_gc_root_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
...
static WASMOON_GUEST_ABI int nested_trap_probe(
    jit_context_t *ctx,
    int64_t *values,
    void *func_ptr
) {
