...
- [x] Across at least seven clean current-version captures on one host, AEGIS-128L bundle allocation has a median at or below 15 ms and function 56 register allocation has a median at or below 15 ms.
- [x] The optimization does not materially increase function 56 code size from the recorded 16,304-byte baseline or regress the BLAKE2b and Salsa20 code-quality controls.
...
- 2026-08-28: ISS-443 replaced the nested VCode allocation adapter with flat
  tables exposed as read-only spans. Seven AEGIS-only captures reduce median
  aggregate register allocation from 53,060 to 51,725 us and function 56 from
  22,375 to 22,088 us without changing allocator decisions or emitted bytes.
  This closes the data-boundary prerequisite for ISS-444; the bundle loop
  remains the dominant 25,328 us aggregate phase.

- 2026-08-28: Continued the AEGIS-only attribution without running the full
  algorithms corpus. A temporary classification build accounted for every one
  of function 56's 603,391 occupied-range visits: 39,915 (6.6%) advance an
  occupied segment that ends before the current bundle segment, 304,112
  (50.4%) advance a bundle segment that ends before the occupied segment, and
  259,364 (43.0%) inspect an overlap. Making either side reseek more
  aggressively reduced visits but made compilation slower, and caching
  clobber coordinates likewise had no net benefit; all three experiments were
  reverted.

  The retained change removes reference-counted `AllocationSegment` reads from
  the scan-only path. The loop now carries segment IDs through cached endpoint
  comparisons and materializes the two segment records only for the 4,620
  previously unseen conflicts that need their range starts. Across seven clean
  cold-cache captures, median module compilation fell from 170,719 to 161,372
  us, aggregate register allocation from 68,307 to 62,377 us, and aggregate
  bundle allocation from 32,860 to 29,084 us. Function 56 register allocation
  fell from 28,778 to 26,269 us and its bundle allocation from 16,592 to
  14,709 us. The deterministic loop counts and 16,036-byte function size are
  unchanged. The per-function bundle target is now met, but the issue remains
  open because function 56's complete register allocation is still above 15
  ms and the aggregate bundle phase remains above 15 ms.
...
- 2026-09-03: Seven clean current-version AEGIS-128L captures close the
  replacement canonical bounds. Aggregate bundle allocation has a 6,440 us
  median and function 56 complete register allocation has a 5,206 us median,
  both far below 15 ms. Median module compilation is 55,570 us and aggregate
  regalloc is 19,613 us. Function 56 is 15,060 bytes versus the 16,304-byte
  opening baseline, while the bounded BLAKE2b and Salsa20 controls are 6,768
  and 4,072 bytes with no allocation-pressure regression. Every capture exits
  successfully and emits the same 58,576-byte module code vector.

## Close Notes

Closed after the sequence of verifier gating, reusable allocator state, dense
hot-path representation, indexed occupancy traversal, and liveness cleanup
reduced the canonical AEGIS allocator below both current 15 ms limits without
trading away code quality. The final strict native suite passes 2215/2215 with
`VCODE_REGALLOC_VALIDATION=1`; warning-73, generated interfaces, formatting,
and module-boundary checks pass. The unrecovered original reporter artifact is
not used as closure evidence; the issue's documented checked-in AEGIS
replacement is.
