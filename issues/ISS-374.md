...
- 2026-08-02: Third review round closed the remaining ownership bypass:
  `ComponentLinker::register` was public and accepted any instance, so
  external code could enlist a foreign instance into the owned list that
  terminal close destroys. Registration is no longer public API at all —
  instantiation frames stage internally and only a successful root commits —
  so the owned list can only ever contain instances this linker created.
  The audit pins the absence of a public register with a negative gate
  test.
