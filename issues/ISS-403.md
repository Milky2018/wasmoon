...
Use one target-neutral source-local materialization policy in MilkIR. Keep
operand-free integer constants that both current targets materialize cheaply,
and their direct unsigned extensions, out of global CSE. They remain local
source recipes instead of becoming function-long SSA values. Do not add
distance thresholds, clone caps, general recursive recipes, or allocator
support.
...
- [x] The 256-distinct-address fixture stays at or below 16 B/op at 400 ops
      and beyond, with no cliff between 280 and 300.
- [x] Spill and reload traffic is well below the measured 64.5% share of
      emitted bytes.
- [x] Constants and `EnvironmentField` values go through one
      target-independent mechanism rather than two.
- [x] Workspace tests and the WAST corpus pass in both modes.
...
- 2026-08-12: The first correction moved the proposed mechanism after global
  GVN, but end-to-end measurement then showed that block-local rebuilding was
  still the wrong granularity for this one-block workload.
...
- 2026-08-12: ISS-407's AArch64 displacement fold reduces those same fixtures
  from a stable 20 B/op after ISS-405 to a stable 12 B/op, with no address
  constant materializations. ISS-405 and ISS-407 measurements remain separate.

- 2026-08-12: ISS-406 removed AArch64's separate environment-field alias and
  pressure heuristics. Stable fields remain separate through the existing
  MilkIR GVN policy, and both AArch64 and x64 materialize each semantic MachV
  occurrence without target-local reuse policy.

- 2026-08-12: ISS-418 removed the e-graph `remat_set`, marking rules, and cost
  bonus. They selected expressions but never rebuilt values or shortened live
  ranges, so retaining them beside ISS-405 would misrepresent the active
  policy.
...
- Cheap integer constants and direct unsigned extensions remain source-local
  recipes instead of becoming function-long SSA definitions.
- AArch64 folds supported constant memory offsets into load/store
  displacements; the 280/300/400/800-operation probes emit
  3,380/3,620/4,820/9,620 bytes (12.07/12.07/12.05/12.03 B/op), with zero
  address constants, spills, or reloads.
- Stable environment fields use the same source-local GVN policy on AArch64
  and x64; the AArch64-only alias, distance, and pressure heuristics are gone.
- Removed the unused regalloc rematerialization planner and the e-graph's
  metadata-only rematerialization marks.
- `moon check --target native --warn-list +73` and
  `moon test --target native` passed, 2635/2635. The recursive WAST corpus
  passed in both interpreter and JIT modes, 258/258 files and 62,563/62,563
  assertions per mode.
