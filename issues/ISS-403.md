...
The first policy is deliberately narrow: clone only operand-free integer
constants that both current targets materialize cheaply. Clone once per using
block, rewrite instruction and terminator uses, then run DCE. Do not add
distance thresholds, clone caps, recursive recipes, or allocator support.
