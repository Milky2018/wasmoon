...
- [ ] A checked-in `x += 3` reproducer that forces spilling compiles with an
      in-place add and no back-edge move on both targets.
- [ ] The `examples/algorithms` corpus compiles and runs unchanged or faster;
      no workload regresses beyond noise on min-of-5.
- [ ] Focused regalloc tests cover the spilling positive case and a negative
...
- 2026-08-12: ISS-408 and ISS-416 substantially changed the allocator's range
  indexing, splitting, spillsets, and hot-loop data structures, but neither
  added the missing dual-target spilling reproducer for this issue. The
  closure gap remains behavioral rather than architectural: capture the
  original pressured loop, then prove the positive and genuine-conflict cases
  on both targets before changing status.

- 2026-08-10: **Reopened after auditing the closure evidence.** The 2026-08-03
  measurements cover only AArch64 and explicitly say that neither substitute
  reproducer spills. They therefore do not prove the original spilling shape
  or the required x64 result. The dual-target positive/negative pipeline tests
  added by `db1bbf88` cover branch threading with two non-spilling carried
  values; ISS-263's forced-spill regression covers canonical spill-slot sharing.
  Neither is a focused regression for this issue's spilling register-pressure
  case. No min-of-5 `examples/algorithms` corpus result is recorded either.
  All acceptance criteria are unchecked until those artifacts exist.
...
Keep open. Current modules/regalloc/allocation_plan_test.mbt covers conflict-free loop edges, but its loop-transfer example has an empty spill_slots result. This does not establish the required spilling x += 3 positive case, genuine-conflict control and emitted-code evidence on both targets. No new min-of-5 algorithm-corpus measurements were made in this audit. Native suite success is not a coalescing/performance acceptance result.
