...
## 12) Alignment Ledger (Living)

This ledger tracks concrete optimization alignment work against Cranelift and is
updated when related tasks close.

| Task | Area | Before | After | Evidence |
|---|---|---|---|---|
...
| `wasmoon-il8.10` | internal compiler validation gate | MilkIR, selected VCode, and allocation verifiers repeated checks over compiler-owned data in the production JIT. | Safe public APIs still validate by default; Wasmoon production uses trusted internal paths, while `VCODE_REGALLOC_VALIDATION=1` restores every check in CI. This matches Wasmtime's production policy of disabling Cranelift's internal verifier. | `modules/wasmoon_jit/regalloc_validation.mbt`, `modules/milkir/native/lower.mbt`, `modules/aarch64_target/direct_lower.mbt`, `modules/x64_target/direct_lower.mbt`, `.github/workflows/check.yml` |
...
On 2026-09-01, seven cold `box_seed.wasm` metric captures measured the
`wasmoon-il8.10` production path at a 105,770 us median module compile time,
down from 131,206 us (-19.4%). IR size, allocation counters, and the 124,968-byte
code-size vector were unchanged. A strict capture restored MilkIR, selected
VCode, and allocation verification.

**Latest local validation snapshot (2026-02-06, darwin/arm64):**
