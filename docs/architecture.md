...
Register allocation does not own or copy a second machine-IR graph. `machv_regalloc` adapts the selected target VCode to the reusable allocator's read-only `FunctionView`; both production targets explicitly select the backtracking strategy. The aggregate target pipeline verifies selected VCode before allocation and independently verifies the materialized VCode allocation before frame planning. `Fast` remains an explicit library option and is not a production fallback.
