...
def main() -> None:
  ...
  (import "wasi_snapshot_preview1" "fd_prestat_get"
    (func $preopen (param i32 i32) (result i32)))
  (memory (export "memory") 1)
  (data (i32.const 16) "isolated-install-ok\n")
  (func (export "_start")
    (if (call $args (i32.const 64) (i32.const 68)) (then unreachable))
    (if (i32.ne (i32.load (i32.const 64)) (i32.const 2)) (then unreachable))
    (if (call $env (i32.const 64) (i32.const 68)) (then unreachable))
    (if (i32.ne (i32.load (i32.const 64)) (i32.const 1)) (then unreachable))
    (if (call $preopen (i32.const 3) (i32.const 64)) (then unreachable))
    (if (i32.ne (i32.load (i32.const 68)) (i32.const 8)) (then unreachable))
...
''')
        for engine in [[], ['--no-jit']]:
            result = subprocess.run([str(executable), 'run', str(guest), *engine,
                                     '--env', 'WASMOON_PROBE=shared',
                                     '--dir', str(stage) + '::/sandbox', '--', 'argument'],
