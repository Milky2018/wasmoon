...
def has_validated_instantiation_boundary(
    ...
    """Check that validation evidence exposes only immutable component bytes."""
    ...
    has_immutable_snapshot_accessor = re.search(
        r"pub fn ValidatedComponent::component_bytes\(Self\) -> Bytes",
        validator,
    )
    ...
    return bool(
        ...
        and has_immutable_snapshot_accessor
...
def audit_repo(root: Path) -> list[AuditCheck]:
    ...
    checks.append(
        AuditCheck(
            ...
            "validation, the evidence must expose only immutable bytes, "
